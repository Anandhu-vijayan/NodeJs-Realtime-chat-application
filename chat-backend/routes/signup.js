...
router.post('/signup', async (req, res) => {
  ...
  try {
    const checkQuery = `SELECT * FROM chat.user_details WHERE email = $1`;
    ...
    if (checkResult.rows.length > 0) {
      ...
      const checkExist = `SELECT * FROM chat.otp_management WHERE email = $1`;
      const checkExsResult = await pool.query(checkExist, [emailId]);
      if (checkExsResult.rows.length === 0) {
        const insertQuery = `
